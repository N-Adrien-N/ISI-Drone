...
import action.ActionNewProduct;
...
public class NewProduct extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        ///Affichage de l'interface newProduct.jsp
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ActionNewProduct.newProduct(request, response)) {
            request.getRequestDispatcher(Const.PATH_PAGE_NEWPRODUCT_COMPLETE).forward(request, response);
        } else {
            processRequest(request, response);
        }

        // 1. Recuperation de params
        // 2. Traitement
        // 3. Renvoi vers JSP (interface)
    }
